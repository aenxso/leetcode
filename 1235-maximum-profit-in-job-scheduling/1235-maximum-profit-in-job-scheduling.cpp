...
public:
    int jobScheduling(vector<int>& startTime, vector<int>& endTime, vector<int>& profit) {
        // generate the sorting index such that endTime[index[i]] <= endTime[index[i+1]] for 0 <= i < n-1
        // to perform binary search, sort the endTime array. Then calculate dp table
