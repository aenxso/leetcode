...
private:
    ...
    void dfs(vector<vector<int> >& mat, vector<vector<bool> >& visited, int i, int j){        
        ...
        if(i + 1 <  m && mat[i + 1][j] >= mat[i][j]) dfs(mat, visited, i + 1, j); /*⬇️*/
        if(i - 1 >= 0 && mat[i - 1][j] >= mat[i][j]) dfs(mat, visited, i - 1, j); /*⬆️*/
        if(j + 1 <  n && mat[i][j + 1] >= mat[i][j]) dfs(mat, visited, i, j + 1); /*➡️*/
        if(j - 1 >= 0 && mat[i][j - 1] >= mat[i][j]) dfs(mat, visited, i, j - 1); /*⬅️*/
    }
};